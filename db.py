...
from sqlalchemy import create_engine, String, Integer, DateTime, Numeric, Boolean, inspect, text
...
def _migrate_history(conn):
    """Normalize known legacy history schemas to order_id/status/created_at.

    This rebuilds only order_status_history. It preserves IDs and timestamps and
    never touches orders, users, settings, or comments.
    """
    ...
    if "orders" in tables:
        order_states = {
            ...
            for r in conn.exec_driver_sql('SELECT id,status,created_at FROM "orders"').mappings().all()
        }

    conn.exec_driver_sql('DROP TABLE IF EXISTS "order_status_history_new"')
    conn.exec_driver_sql('''CREATE TABLE "order_status_history_new" (
    ...
    for r in rows:
        status = r.get("status") or r.get("new_status") or r.get("old_status")
        created_at = r.get("created_at")
        if not status and r.get("order_id") in order_states:
            status = order_states[r["order_id"]][0]
        if not created_at and r.get("order_id") in order_states:
            created_at = order_states[r["order_id"]][1]
        conn.exec_driver_sql(
            'INSERT INTO "order_status_history_new" (id,order_id,status,created_at) VALUES (?,?,?,?)',
            (r.get("id"), r.get("order_id"), status or "WAITING_PAYMENT", created_at),
        )
...
def _migrate_comments(conn):
    tables = {r[0] for r in conn.exec_driver_sql("SELECT name FROM sqlite_master WHERE type='table'").fetchall()}
    if "admin_comments" not in tables:
        return
    cols = _columns(conn, "admin_comments")
    additions = {"order_id": "INTEGER", "text": "VARCHAR(1000)", "created_at": "DATETIME"}
